...
class ExternalProviderController extends Controller
{
    ...
    public function index( Request $request ){
        ...
        $digital_type = $this->resourceService->getResourceType(null, 'external-provider');
        ...
        \Breadcrumbs::register('ext-provider/index', function($breadcrumbs) use ($route_params, $organisation) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('External Providers');
        });
        // ------------------------------------------
    	return \View::make('app.resources.index-ext-provider',$data);
    }

    public function add( Request $request ){
        $organisation = $this->orgService->getOrganisation(null, \Request::segment(3));
        $data = array();
        $data['resource_type'] = $this->resourceService->getResourceType( null, 'external-provider');
        // ------------------------------------------
        // Create the breadcrumb trail for this view
        $route_params['account_name'] = parent::getDefaultRouteParams($request, ['account_name'=>1]);
        $route_params['all'] = parent::getDefaultRouteParams($request);
        \Breadcrumbs::register('ext-provider/new', function($breadcrumbs) use ($route_params, $organisation) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('External Providers', route('ext-provider/index', $route_params['all']));
            $breadcrumbs->push('New');
        });
        // ------------------------------------------
        return \View::make('app.resources.new-ext-provider',$data);
    }
    ...
    public function submit( ResourceRequest $request){
    	...
    	return ($submitted) ? redirect()->route('ext-provider/index', $route_params) : 'Resource Not Created';
    }
     ...
     */
     ...
     public function view(Request $request){
        ...
        $route_params['account_name'] = parent::getDefaultRouteParams($request, ['account_name'=>1]);
        $route_params['all'] = parent::getDefaultRouteParams($request);
        \Breadcrumbs::register('ext-provider/view', function($breadcrumbs) use ($route_params, $organisation, $data ) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('External Providers', route('ext-provider/index', $route_params['all']));
            $breadcrumbs->push($data['resource_data']->name);
        });
        // ------------------------------------------
        return \View::make('app.resources.view-ext-provider',$data);
     }
     ...
     */
     ...
     public function update(ResourceRequest $request){
        ...
        // Generate a url for the return call
        $route_params = parent::getDefaultRouteParams($request);
    	return ($updated) ? redirect()->route('ext-provider/index', $route_params) : 'Resource Not Updated';
     }
     ...
     */
      ...
      public function destroy(Request $request){
        ...
        // Generate a url for the return call
        $route_params = parent::getDefaultRouteParams($request);
      	return ($deleted) ? redirect()->route('ext-provider/index', $route_params) : 'Resource can\'t be Deleted';
      }
