...
class LocationResourceController extends Controller
{
    ...
    public function index( Request $request ){
        ...
        $digital_type = $this->resourceService->getResourceType(null, 'location');
        ...
        \Breadcrumbs::register('locations/index', function($breadcrumbs) use ($route_params, $organisation) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('Rooms and Locations');
        });
        // ------------------------------------------
    	return \View::make('app.resources.index-location',$data);
    }
    ...
    public function add( Request $request ){
        $organisation = $this->orgService->getOrganisation(null, \Request::segment(3));
        $data = array();
        $data['resource_type'] = $this->resourceService->getResourceType( null, 'location');
        // ------------------------------------------
        // Create the breadcrumb trail for this view
        $route_params['account_name'] = parent::getDefaultRouteParams($request, ['account_name'=>1]);
        $route_params['all'] = parent::getDefaultRouteParams($request);
        \Breadcrumbs::register('locations/new', function($breadcrumbs) use ($route_params, $organisation) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('Rooms and Locations', route('locations/index', $route_params['all']));
            $breadcrumbs->push('New');
        });
        // ------------------------------------------
        return \View::make('app.resources.new-location',$data);
    }
    ...
    public function submit( ResourceRequest $request){
    	...
    	return ($submitted) ? redirect()->route('locations/index', $route_params) : 'Resource Not Created';
    }
     ...
     */
     public function view(Request $request){
        ...
        $route_params['account_name'] = parent::getDefaultRouteParams($request, ['account_name'=>1]);
        $route_params['all'] = parent::getDefaultRouteParams($request);
        \Breadcrumbs::register('locations/view', function($breadcrumbs) use ($route_params, $organisation, $data ) {
            $breadcrumbs->push($organisation->name, route('orgs/view', $route_params['all']));
            $breadcrumbs->push('Rooms and Locations', route('locations/index', $route_params['all']));
            $breadcrumbs->push($data['resource_data']->name);
        });
        // ------------------------------------------
        return \View::make('app.resources.view-location',$data);
     }
     ...
     */
     public function update(ResourceRequest $request){
        ...
        // Generate a url for the return call
        $route_params = parent::getDefaultRouteParams($request);
    	return ($updated) ? redirect()->route('locations/index', $route_params) : 'Resource Not Updated';
     }
     ...
     */
      public function destroy(Request $request){
        ...
        // Generate a url for the return call
        $route_params = parent::getDefaultRouteParams($request);
      	return ($deleted) ? redirect()->route('locations/index', $route_params) : 'Resource can\'t be Deleted';
      }
